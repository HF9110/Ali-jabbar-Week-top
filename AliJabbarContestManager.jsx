...
// استخدام متغيرات البيئة بدلاً من إدراج المفتاح مباشرة في الكود
const VITE_FIREBASE_API_KEY = typeof process.env.VITE_FIREBASE_API_KEY !== 'undefined' 
    ? process.env.VITE_FIREBASE_API_KEY 
    : "AIzaSyDUxC_2orwmSLL9iEBIkeohZKfH36MjZ4Y"; // القيمة الافتراضية القديمة (لتشغيل التطبيق)
...
    // هذا سيحدث إذا لم يكن مفتاح API صحيحاً أو موجوداً
...
            // If running in non-Firebase environment, userId will be 'mock-user-id'
...
    useEffect(() => {
        if (!db || !isAuthReady) {
            // Handle mock mode or unready state gracefully if needed, 
            // but since we call initializeFirestore, we rely on the logic inside initDataRef useEffect.
            return;
        }
...
                        // setSettings={(setter) => setSettings(setter)} // Removed unused prop
                    />
                ) : (
                    <Home 
                        settings={settings} 
...
            
            <AdminAuthModal 
                isOpen={authModalOpen && !adminMode} // Only open if not already authenticated
